...
let fileXiter g path = 
    use s = File.OpenText path
    while not (s.EndOfStream) do g s
    s.Close()

let fileFold f e s = 
    fileXfold (fun e s -> f e (s.ReadLine())) e s

let fileIter g s = fileXiter (fun s -> g (s.ReadLine()) ) s
...
type orderString =
    ...
    interface System.IComparable with
        member s1.CompareTo sobj = 
            match sobj with
            | :? orderString as s2 ->
                if s1.Cult <> s2.Cult then raise StringOrderingMishmatch
                else 
                match s1.Cmp s1.Str s2.Str with
                | 0 -> compare s1.Str s2.Str
                | z -> z
            | _ -> invalidArg "sobj"
                              "can not compare values with different types"
...
let orderString (cult: string) =
    let cultInfo = CultureInfo cult
    let comp s1 s2 = 
        String.Compare(s1, s2, cultInfo, CompareOptions.None)
